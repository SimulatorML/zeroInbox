...
- **Бизнес-требования:**
    - Создание бота для категоризации и поиска сохраненных сообщений в Telegram.
...
- **Итерации проекта**
    - Первая итерация:
        - Создание базовой версии для проверки `POC`, с категоризацией сообщений по предопределенным классам и возможностью поиска по ним.

    - Вторая итерация:
        - Реализация полноценного MVP сервиса с интеграцией в Telegram и возможностью создавать и назначать собственные категории, удобным интерфейсом поиска и просмотра сообщений
...
- **Бизнес-процесс пилота:**
    - Пользователи тестируют бота, используя его для категоризации и поиска своих сохраненных сообщений.
...
### 1.4 Что входит в цель проекта/итерации, что не входит
- **В цель входит:**
    - Разработка функционала категоризации и поиска, интеграция с Telegram API

    - Тестирование с реальными пользователями.

    - Покрытие кода тестами с помощью `pytest`

- **Не входит:**
    - Расширенные функции аналитики сообщений, интеграция с внешними сервисами кроме Telegram.
...
- Создание удобного инструмента для работы с избранными сообщениями, который позволит пользователю быстро находить нужную информацию.
...
- **Этап 1:**
    ...
    - Использование базы данных (PostgreSQL) для хранения категоризированных данных.

- **Этап 2:**
    - Подготовка тестовых данных: Скраппинг реддита, в качестве сообщений - заголовки постов, а наименование темы поста (topic name) в качестве категории.

    - Тестирование:
        - Проверка работоспособности бота на тестовой выборке данных.

        - Учитывая тот факт, что одно и тоже сообщение можно отнести к
        разным классам (например `programming` или `technology`) для оценки качества классификации будет использоваться метрика `Recall` для бинарной классификации, где неопределенный класс(`unknown`) получает метку 0, остальные классы - 1.

- **Этап 3:**
    - Пользовательский интерфейс: Разработка функционала для просмотра категоризированных сообщений.

    - Промпт инженерия (для нивелирования количества `unknown` классов)

    - Проверка альтернативных подходов к классификации (Semantic Router: https://github.com/aurelio-labs/semantic-router).


- **Этап 4:**
    - Расширенный пользовательский интерфейс: Добавление пользовательских категорий, возможности поиска по категориям, фильтрации и сортировки сообщений.

    - Развертывание чат-бота.

    - Запуск MVP среди ограниченной группы пользователей для сбора обратной связи и дальнейшего улучшения продукта.
